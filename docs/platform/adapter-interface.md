...
| LocalAdapter | Moves artifacts to cycle-scoped archive locations under the cycle's directory. Updates each moved node's location entry in the index to reflect its new location. Deletes stale index entries for any artifacts no longer present at their original location after the move. |
...
| Atomicity | LocalAdapter performs all moves and index updates atomically — on failure, any moves already completed are rolled back before the error is thrown. RemoteAdapter relies on server-side transaction semantics. |
