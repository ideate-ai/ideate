...
// claim/renew/complete/release with fencing tokens.
//
// Implements claim semantics — the heart of the contract: actor attribution,
// fencing, a unified error base, and transition+event atomicity (see the
// notes on each function below). This module builds directly on the storage
// primitives store.ts deliberately leaves out of scope (store.ts's own
// header): claim acquisition/renewal/completion semantics (compare-and-set,
// lease expiry, cycle detection) are built on top of those primitives here.
//
// Every entry point below follows the same three-step shape, in this exact
// order, ALL THREE inside one `BEGIN IMMEDIATE ... COMMIT` unit on one
// connection (otherwise step 3 could commit on a SEPARATE connection after
// step 2 had already committed and closed, so a crash between the two could
// leave a transition with no event):
//   1. `checkExpiry` (expiry.ts) — the lazy check, evaluated FIRST (every
//      verb touching an item first evaluates expiry). Safe to call
//      unconditionally, even for an id that turns out not to exist. Runs
//      BEFORE the transaction below (it is its own atomic unit, in expiry.ts).
...
//      cross-referenced to the rule it implements.
//   3. `appendEventRowOn` (store.ts) — the immutable transition event (every
//      transition appends an immutable event), attributed to the claim's OWN
//      holder (never a caller-supplied `actor` argument for
//      `complete`/`release` — see their own doc comments), appended and
//      committed in the SAME transaction as step 2, only on success. A thrown
//      `ClaimEngineError` means the whole transaction rolled back — the CAS
//      itself did not happen, so there is nothing to record (every failure
//      mode is typed and loud, never silent).
//
// Why direct SQL here rather than a new store.ts primitive: the acquire CAS
// must be verifiably engine-level, not JS-level. `store.nextClaimToken` opens
// its own connection and commits before returning — composing it with a
// second `UPDATE ... SET status = ...` call would be exactly the
// read-then-write race the contract forbids. Every CAS here instead
// increments `claim_token_counter` and sets `claim_token` to the SAME new
// value inside ONE statement, using the identical counter column store.ts
// defined (schema.ts's `items.claim_token_counter`) — the store's counter,
// just incremented via one round trip instead of two.
//
// Depends_on gating (a claim succeeds iff all depends_on are done): the
 ...
 *  failure modes that have no analogue in the store's own
 *  `WorkStateErrorCode` union get their own typed error family here rather
 *  than an unsafe cast into that union). */
export type ClaimEngineErrorCode =
   ...
   *  and a renew arriving after expiry (the lazy check already flipped the
   *  item back to `open` by the time this verb's own CAS runs, so "not
   *  in_progress" and "post-expiry" are the same rejection). */
  | 'INVALID_CLAIM';
 ...
 *  ...Error, never silent"). Extends `WorkStateModuleError`
 ...
 *  bug (the contract's posture is 'hours, not seconds', and a month-long
 *  lease defeats orphan recovery entirely). Without this validation, an
 *  oversized leaseMs crashed claim/renew with an untyped RangeError, and a
 *  non-positive value produced a claim born already expired whose own holder
 *  was then rejected with a misleading 'reclaimed by another actor'
 *  diagnostic. */
 ...
 *
 * `checkExpiry` runs first so that an item whose
...
export function claim(
  ...
  try {
    // The CAS and its `claim` audit event commit as ONE atomic unit — a
    // crash between "claim acquired" and "event appended" can no longer leave
    // a transition without its event. The BEGIN IMMEDIATE/COMMIT/ROLLBACK
    // boilerplate lives in tx.ts's `withWriteTransaction`, which also re-types
    // an exhausted busy_timeout as `WorkStateError('BUSY', ...)` — the CAS's
    ...
    withWriteTransaction(db, (db) => {
      ...
      const row = db
        .prepare(
          ...
          // BYTE-FOR-BYTE (not re-flowed to the closure's JS nesting depth) —
          // claims.test.ts's own structural test asserts on this exact text,
          // clause by clause.
          `UPDATE items
  ...
  if (claimedToken === undefined) {
    ...
    // No "item no longer exists" branch here — `requireItem` already proved
    // the row exists before this CAS ran, and nothing in this module (or any
    // sibling) ever deletes an `items` row, so that branch was unreachable
...
/**
 * `renew(id, claim_token)` — the renew rule. Itself a compare-and-set:
...
export function renew(
  ...
  try {
    // Atomicity + actor derivation: the CAS and its `renew` audit event
    // commit as ONE atomic unit; the event's actor is read back off the SAME
    // row the CAS just matched — `claim_holder_*` is untouched by this UPDATE
    // (only `claim_lease_expires` changes), so its `RETURNING` value is
    // exactly the current holder, read within this same locked transaction.
    // `renew(id, claim_token)` carries no actor parameter in its own
    // signature — the token already proves the caller is the current holder.
    // The transaction boilerplate lives in tx.ts's `withWriteTransaction`.
    withWriteTransaction(db, (db) => {
      const row = db
        .prepare(
          `UPDATE items
  ...
  if (renewedHolder === undefined) {
    throw new ClaimEngineError(
      'INVALID_CLAIM',
      `work-state claim engine: renew: item ${JSON.stringify(itemId)} is not held under token ${String(claimToken)} (expired, stale, or item not in_progress) — the claim is gone; re-claim if the item is still open`,
    );
  }
...
/**
 * `complete(id, claim_token, note?)` — the complete rule. Succeeds only with
 * the CURRENT token — a worker whose lease expired and was reclaimed by
 * someone else holds a stale token and is rejected (the Kleppmann
 * delayed-writer test): fencing falls out of the CAS's own `claim_token = ?`
 * condition, not a separate check.
 *
 * NO `actor` parameter: the completion is always attributed to the claim's
 * OWN holder — read within the SAME locked transaction as the CAS itself,
 * never a caller-supplied argument. Otherwise any token holder (including a
 * stale one, if this parameter were accepted independently of the fencing
 * check) could misattribute the audit event to an arbitrary actor;
 * accountability must resolve to the actual holder, not to whatever the
 * caller claims. Because the completing UPDATE itself clears `claim_holder_*`
 * (so `RETURNING` would yield the POST-image, i.e. already-NULL, holder), the
 * holder is read via a preliminary `SELECT` gated by the identical
 * `status`/`claim_token` predicate, inside the same `BEGIN IMMEDIATE` unit as
 * the UPDATE — not a separate, unguarded read.
 *
 ...
 *
 * `completionRecord` (optional): when a transport supplies it, this
 ...
 * committed — the ordering requirement (board = state authority, record
...
export function complete(
  ...
  try {
    // The CAS and its `complete` audit event commit as ONE atomic unit. The
    // transaction boilerplate lives in tx.ts's `withWriteTransaction` — see
    // this function's own doc comment for why the completion-record hook below
    // still fires strictly AFTER this call returns (i.e. after commit),
    // unchanged.
    withWriteTransaction(db, (db) => {
      const pre = db
        .prepare(
  ...
  if (completedBy === undefined) {
    throw new ClaimEngineError(
      'INVALID_CLAIM',
      `work-state claim engine: complete: item ${JSON.stringify(itemId)} is not held under token ${String(claimToken)} (stale token — reclaimed by another actor, or item not in_progress)`,
    );
  }
  const completedItem = requireItem(store, itemId, 'complete');

  // The completion-record post-commit hook — strictly AFTER the CAS
...
/**
 * `release(id, claim_token, note?)` — the release rule. Token-checked;
 * returns the item to `open` with a free-text handoff note appended to the
 * event log. `note` is optional (mirroring `complete`) — a handoff note is
 * the common case, not a hard requirement; an absent note still records the
 * transition.
 *
 * NO `actor` parameter (mirrors `complete`'s own behavior): the release is
 * always attributed to the claim's OWN holder, read within
...
export function release(store: WorkStateStore, clock: Clock, itemId: string, claimToken: number, note?: string): WorkItem {
  ...
  try {
    // The CAS and its `release` audit event commit as ONE atomic unit. The
    // transaction boilerplate lives in tx.ts's `withWriteTransaction`.
    withWriteTransaction(db, (db) => {
      const pre = db
        .prepare(
          `SELECT claim_holder_human, claim_holder_agent FROM items
  ...
  if (releasedBy === undefined) {
    throw new ClaimEngineError(
      'INVALID_CLAIM',
      `work-state claim engine: release: item ${JSON.stringify(itemId)} is not held under token ${String(claimToken)} (stale token, or item not in_progress)`,
    );
  }
...
// hybrid-expiry mechanism) — re-exported here so callers of this module's
// `LeaseOptions` have one source of truth for the default lease length,
// "hours not seconds", without importing expiry.ts directly.
