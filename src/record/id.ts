...
const ENCODING_LOOKUP: ReadonlyMap<string, number> = new Map(
  [...ENCODING].map((c, i) => [c, i] as const),
);
 ...
 */
export function createUlidGenerator(clock: Clock): UlidGenerator {
  // Per-session entropy: each generator instance owns its own entropy state,
  // seeded independently at construction.
  let lastTime = -1;
  let lastRandom: Uint8Array = randomBytes(RANDOM_BYTES);

  return function generateUlid(): string {
...
export function parseUlidTimestamp(ulid: string): Date {
  ...
  let timeMs = 0;
  for (let i = 0; i < TIME_CHARS; i++) {
    const digit = ENCODING_LOOKUP.get(ulid[i] as string);
    if (digit === undefined) {
