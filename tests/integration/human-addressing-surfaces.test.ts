...
//
// "EXISTING under the prose roots" means SHIPPED, not "present on disk under
// that path at all": `docs/architecture/build/` holds mermaid-processed
// COPIES of six of this census's own source docs, generated by
// `docs/architecture/render.sh` and gitignored — never shipped, and no
// per-file exemption reason is even coherent for `combined.md`, which
// concatenates exempt and non-exempt source content into one file. The walk
// this census is built on (`listShippedMarkdownFilesUnderRoots`, in
// shipped-markdown-registry.ts, shared with the other three registry-scanning
// suites) excludes generated output MECHANICALLY, at the walk, before file
// existence is ever asked about — see that module for which oracle decides
// shipped-ness and why.
//
...
const CENSUS_ROOTS = SHIPPED_PROSE_ROOTS;
...
const EXEMPTIONS: Record<string, string> = {
  ...
  'docs/architecture/00-overview.md':
    'Authored architecture inventory doc for a human reader (stage-3 alignment input); same standing as docs/workflow-guide.md.',
  'docs/architecture/01-process-record.md':
    'Authored architecture inventory doc for a human reader; same standing as docs/workflow-guide.md.',
  'docs/architecture/02-delegation-board.md':
    'Authored architecture inventory doc for a human reader; same standing as docs/workflow-guide.md.',
  'docs/architecture/03-steering.md':
    'Authored architecture inventory doc for a human reader; same standing as docs/workflow-guide.md.',
  'docs/architecture/04-transports-and-infra.md':
    'Authored architecture inventory doc for a human reader; same standing as docs/workflow-guide.md.',
  'docs/architecture/05-usage-audit.md':
    'Authored architecture inventory doc for a human reader; same standing as docs/workflow-guide.md.',
};

/** Every SHIPPED .md under the prose roots, as forward-slash relative paths
 *  (see shipped-markdown-registry.ts for what "shipped" excludes and why). */
function deriveCensus(base: string): string[] {
  return listShippedMarkdownFilesUnderRoots(base, CENSUS_ROOTS).sort();
}
