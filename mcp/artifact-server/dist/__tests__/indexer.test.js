 ...
 */
function makeIdeateDir(baseDir) {
...
function minimalWorkItem(overrides = {}) {
    const defaults = {
        id: "WI-001",
        type: "work_item",
...
describe("rebuildIndex — work item YAML → table populated", () => {
    it("inserts a work item row with correct title", () => {
        const db = freshDb();
        const ideateDir = makeIdeateDir(tmpDir);
        writeYaml(path.join(ideateDir, "work-items"), "WI-001.yaml", minimalWorkItem({ id: "WI-001", title: "Test work item" }));
...
describe("rebuildIndex — depends edge extracted", () => {
    it("creates a depends_on edge from WI-002 to WI-001", () => {
        ...
        writeYaml(wiDir, "WI-002.yaml", minimalWorkItem({ id: "WI-002", title: "Second item", depends: ["WI-001"] }));
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'WI-002' AND target_id = 'WI-001' AND edge_type = 'depends_on'`)
            .get();
...
describe("rebuildIndex — incremental skip for unchanged file", () => {
    it("returns files_updated: 0 on second call with same content", () => {
        const db = freshDb();
        const ideateDir = makeIdeateDir(tmpDir);
        writeYaml(path.join(ideateDir, "work-items"), "WI-001.yaml", minimalWorkItem());
        rebuildIndex(db, drizzle(db), ideateDir);
...
describe("rebuildIndex — incremental update for changed file", () => {
    it("updates the row and returns files_updated: 1 after content change", () => {
        const db = freshDb();
        const ideateDir = makeIdeateDir(tmpDir);
        const wiPath = writeYaml(path.join(ideateDir, "work-items"), "WI-001.yaml", minimalWorkItem({ title: "Original title" }));
        rebuildIndex(db, drizzle(db), ideateDir);
...
describe("rebuildIndex — stale row deletion", () => {
    it("removes the row when the YAML file is deleted", () => {
        const db = freshDb();
        const ideateDir = makeIdeateDir(tmpDir);
        const wiPath = writeYaml(path.join(ideateDir, "work-items"), "WI-001.yaml", minimalWorkItem());
        rebuildIndex(db, drizzle(db), ideateDir);
...
describe("rebuildIndex — node_file_refs from scope", () => {
    it("inserts a node_file_refs row for each scope entry path", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const ref = db
            .prepare(`SELECT * FROM node_file_refs WHERE node_id = 'WI-001' AND file_path = 'src/foo.ts'`)
            .get();
...
describe("rebuildIndex — blocks edge extracted", () => {
    it("creates a blocks edge from WI-002 to WI-001", () => {
        ...
        const wiDir = path.join(ideateDir, "work-items");
        writeYaml(wiDir, "WI-001.yaml", minimalWorkItem({ id: "WI-001" }));
        writeYaml(wiDir, "WI-002.yaml", minimalWorkItem({ id: "WI-002", title: "Blocker item", blocks: ["WI-001"] }));
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'WI-002' AND target_id = 'WI-001' AND edge_type = 'blocks'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — belongs_to_module edge extracted", () => {
    it("creates a belongs_to_module edge from WI-001 to MOD-core", () => {
        ...
        const ideateDir = makeIdeateDir(tmpDir);
        const wiDir = path.join(ideateDir, "work-items");
        writeYaml(wiDir, "WI-001.yaml", minimalWorkItem({ id: "WI-001", module: "MOD-core" }));
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'WI-001' AND target_id = 'MOD-core' AND edge_type = 'belongs_to_module'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — belongs_to_domain edge extracted", () => {
    it("creates a belongs_to_domain edge from WI-001 to workflow", () => {
        ...
        const ideateDir = makeIdeateDir(tmpDir);
        const wiDir = path.join(ideateDir, "work-items");
        writeYaml(wiDir, "WI-001.yaml", minimalWorkItem({ id: "WI-001", domain: "workflow" }));
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'WI-001' AND target_id = 'workflow' AND edge_type = 'belongs_to_domain'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — derived_from edge extracted", () => {
    it("creates a derived_from edge from DP-001 to GP-001", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'DP-001' AND target_id = 'GP-001' AND edge_type = 'derived_from'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — relates_to edge extracted", () => {
    it("creates a relates_to edge from FIND-001 to WI-001", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'FIND-001' AND target_id = 'WI-001' AND edge_type = 'relates_to'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — supersedes edge extracted", () => {
    it("creates a supersedes edge from DD-002 to DD-001", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'DD-002' AND target_id = 'DD-001' AND edge_type = 'supersedes'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — addressed_by edge extracted", () => {
    it("creates an addressed_by edge from FIND-002 to WI-001", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'FIND-002' AND target_id = 'WI-001' AND edge_type = 'addressed_by'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — amended_by edge extracted", () => {
    it("creates an amended_by edge from DP-001 to DP-002", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'DP-001' AND target_id = 'DP-002' AND edge_type = 'amended_by'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — phase with project field creates belongs_to_project edge", () => {
    it("creates a belongs_to_project edge from PHASE-001 to PROJ-001", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'PHASE-002' AND target_id = 'PROJ-001' AND edge_type = 'belongs_to_project'`)
            .get();
        expect(edge).toBeDefined();
...
describe("rebuildIndex — work item with phase field creates belongs_to_phase edge", () => {
    it("creates a belongs_to_phase edge from WI-001 to PHASE-001", () => {
        const db = freshDb();
        const ideateDir = makeIdeateDir(tmpDir);
        writeYaml(path.join(ideateDir, "work-items"), "WI-003.yaml", minimalWorkItem({ id: "WI-003", title: "Phase-scoped work item", phase: "PHASE-001" }));
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'WI-003' AND target_id = 'PHASE-001' AND edge_type = 'belongs_to_phase'`)
            .get();
        expect(edge).toBeDefined();
    ...
    it("journal_entry with phase field does NOT produce a belongs_to_phase edge", () => {
        ...
        rebuildIndex(db, drizzle(db), ideateDir);
        const edge = db
            .prepare(`SELECT * FROM edges WHERE source_id = 'JE-001' AND edge_type = 'belongs_to_phase'`)
            .get();
