...
*Scope boundary*: Two work items. WI-168 adds the `document_artifacts` table and registers all 10 unregistered types. WI-169 fixes the `module_spec` migration to parse structured fields. Both are additive; no existing functionality changes. Architecture unchanged. Guiding principles unchanged.
