...
**Context**: Post-review remediation of 6 significant findings from the 2026-03-11 capstone review of WI 030-038. Also addresses 3 open questions (OQ1, OQ2, OQ6) and a design aside on model agnosticism in agent definitions.

**Q: Guiding principles — still applicable?**
A: Yes, all 12 principles unchanged.

**Q: Triage S1 (brrr proxy-human invocation broken) and S3 (log format conflict) — address now, defer, or dismiss?**
A: Address now. For S1 fix approach: (a) add proxy-human to default-roles.json + use role: "proxy-human" in brrr.

**Q: Triage S2 (role system zero test coverage), S4 (no diff application), S5 (role advisory undisclosed), S6 (README schema mismatches) — address now, defer, or dismiss?**
A: Address now, all.

**Q: OQ1 — Token budget tracking (deferred since cycle 1). Address now (A) or defer to v3 (B)?**
A: A — add to this cycle.

**Q: OQ2 — Remote role enforcement. Implement role resolution in remote worker (A) or document as advisory-only (B)?**
A: B — document that role is advisory-only for remote dispatch.

**Q: OQ6 — Diff application ownership. brrr execute phase (A), manager agent (B), or new work item (C)?**
A: B — assign to manager agent.

**Q: Minor findings — include all (architecture.md, plugin.json version, worker coroutine shutdown, brrr principles-checker working_dir, brrr refinement cap)?**
A: Include all.

**Q (aside): Agent model agnosticism — don't hardcode model: opus in agent definitions. Specified at spawn time. Mechanism: (A) add model param to spawn_session + change agent frontmatter to sonnet default, or (B) prompt-only instruction?**
A: A — add model parameter to spawn_session; agent definitions default to sonnet; skills specify model at spawn time via the model parameter.

---

## Refinement Interview — 2026-03-10

**Context**: User wants to significantly expand the MCP orchestration layer — remote worker support, role-based agent teams, a manager agent, and an autonomous SDLC loop mode (brrr). This is requirement evolution, not review remediation.

**Q: What specifically do you want to iterate on in the MCP component?**
A: Several major additions: (1) remote task distribution — a daemon on a remote machine that the local MCP server can farm work out to; (2) spawned sessions with different roles on a team; (3) a more fleshed-out orchestration layer where teams operate more autonomously; (4) a lighter-weight version of gastown's work orchestration patterns; (5) `/ideate:brrr` — an autonomous SDLC mode that runs plan→execute→review→refine until convergence.

**Q: What does "remote machine" mean? What's the use case?**
A: A different computer on the local network. The remote runs local models (devstral, qwen, etc.) via claude code, not the Anthropic cloud. Use cases: quota exhaustion on the local machine, or running background work while sleeping (user not actively participating). The remote machine handles jobs where time is not a factor.

**Q: How does the remote integration work — polling, push, shared queue?**
A: Local MCP server has a list of configured remote workers. It sends tasks to them and polls periodically for status. The remote API has a contract to return completed work or give a progress report. The host polls the remotes.

**Q: What protocol for remote communication?**
A: HTTP or WebSocket. Gastown was inspiration for the concept, not the architecture.

**Q: What does "different roles" mean concretely?**
A: Roles are named configuration bundles: system prompt preamble, allowed tools, model, max turns, permission mode. The team has workers who do actual tasks (execute, review, refine tasks) and managers who do upkeep — ensuring workers are working, sending work back to the host machine. Manager is an LLM agent with access to predefined inspection tools (Python-based or LLM prompts). Host polls remotes periodically for status check.

**Q: What is the manager role concretely — LLM or infrastructure?**
A: Manager is an LLM agent that works as part of an agent team for direct communication with other agents. It has access to code checks and predefined tools.

**Q: Stopping condition for brrr mode?**
A: Convergence. After the initial plan, run until refinement creates zero findings AND a check of the guiding principles comes up with perfect adherence. Zero violations. It needs to be perfect; can work as long as needed.

**Q: Human-in-the-loop in brrr mode?**
A: An agent acts on behalf of the human to handle Andon events with its best judgement. Full authority — it uses the guiding principles as source of truth. When the human returns, they are prompted and given a full report of all activity since they left. The proxy agent performs the tasks in individual cycle phases.

**Q: How does codebase access work on the remote machine (file transport vs. shared repo)?**
A: The API contract returns the work — polling the remote returns completed work (diffs or file content) or a progress report.

**Q: Authentication and network model?**
A: Internet eventually, but a simple API key is fine for now.

**Q: What's the threshold for "perfect adherence"?**
A: Zero violations. The proxy-human agent performs this check as part of cycle tasks.
