...
export class RecordStore {
    ...
    #clock;
    #nextId;
    constructor(config, projectRoot, telemetry, clock) {
        this.#config = config;
    ...
    append(input) {
        ...
        const point = typeof input?.source?.capture_point === 'string' && input.source.capture_point.length > 0
            ? input.source.capture_point
            : 'unknown';
        const sessionId = typeof input?.source?.session_id === 'string' && input.source.session_id.length > 0
        ...
        const gate = (text) => {
            const result = scanAndMask(text, {
                onRedaction: (pattern, count) => {
                    ...
                    process.emitWarning(`ideate record: secret gate masked ${String(count)} match(es) of ${pattern} before persisting record ${id}`, { code: 'IDEATE_RECORD_REDACTION' });
                },
            });
            ...
            for (const month of this.#listDir(join(this.recordDir, year), MONTH_DIR)) {
                ...
                for (const file of files) {
                    ...
                    try {
                        ...
                        process.emitWarning(`ideate record: skipping unparseable record file ${filePath} (${errorMessage(err)})`, { code: 'IDEATE_RECORD_UNPARSEABLE' });
                        continue;
                    }
                    if (scopeFilter !== undefined && !matchesScope(record, scopeFilter))
                        continue;
                    out.push(record);
                    if (out.length >= limit)
                        return out;
...
function matchesScope(record, needle) {
    const haystacks = [
        record.scope,
        record.kind,
