...
export const RECORD_TOOL_NAMES = ['record_append', 'record_read', 'record_decision'];
 ...
 */
function writeRecord(ctx, params) {
...
function composeDecisionContent(claim, rationale) {
 ...
 */
export function createRecordToolsRegistrar(options = {}) {
    let context;
    ...
    return (server) => {
        server.registerTool('record_append', {
            description: 'Append one process record (a discovery-candidate entry) to the project record. ' +
            ...
            inputSchema: {
                kind: zString.describe('Record kind — open vocabulary: decision | finding | session-outcome | subagent-outcome | commit-boundary | task-completion | …'),
        ...
        server.registerTool('record_read', {
            description: 'Read process records: newest first, optionally scope-filtered (plain substring selection over ' +
        ...
        server.registerTool('record_decision', {
            description: 'Log a decision (the ADR entry point). Sugar for record_append with kind=decision: the decision ' +
