...
function requireString(value, field) {
    if (typeof value !== 'string') {
        throw new RecordSchemaError(field, `record schema: field "${field}" must be present as a string (empty allowed, absence is a schema error); got ${value === undefined ? 'absent' : typeof value}`);
    }
    return value;
}
/**
...
export function validateRecord(input) {
    ...
    if (sourceRaw === null || typeof sourceRaw !== 'object' || Array.isArray(sourceRaw)) {
        throw new RecordSchemaError('source', 'record schema: field "source" must be present as an object (boundary contract §6.2: fields always present)');
    }
    const src = sourceRaw;
...
function parseScalarValue(rawValue, field) {
    let parsed;
    try {
