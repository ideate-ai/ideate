...
export function redactionMarker(patternName) {
...
const wholeMatch = (marker) => marker;
/** Redaction that keeps capture group 1 (leading non-secret context) and masks the rest. */
const keepPrefix = (marker, _match, groups) => `${groups[0] ?? ''}${marker}`;
/**
...
export const SECRET_PATTERNS = [
    ...
    },
    {
        ...
        regex: /\b((?:aws|secret)[\w.-]{0,40}?["']?\s*[:=]{1,2}\s*["']?)([A-Za-z0-9/+]{40})(?![A-Za-z0-9/+=])/gi,
        redact: keepPrefix,
    },
    {
    ...
    },
    {
        ...
        redact: wholeMatch,
        accepts: (match, { entropyThreshold }) => /[a-z]/.test(match) &&
            /[A-Z]/.test(match) &&
            /[0-9]/.test(match) &&
